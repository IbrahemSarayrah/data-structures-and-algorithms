...
const howMuchPencil = (str) => {
  ...
  oldString.forEach((i, idx) => {
    let shorterString = oldString.slice(idx, oldString.length);
    let newString = shorterString.join('');
...
const listFoods = (recipe) => {
  ...
  newArray.map((list,index) => {
    let groceryList = recipe.ingredients[index].indexOf(' ', 6);
    result.push(recipe.ingredients[index].slice(groceryList + 1));
  });
  return result;
};
