...
function transformToLis(obj) {
  ...
  Object.entries(obj).forEach(object => {
    newArray.push(`<li>${object[0]}: ${object[1]}</li>`);
  });
  return newArray;
}
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let newArray = input.map(number =>
    number.filter(num =>
      (typeof num === 'number' && num % 5 === 0))
      .map(newNumber =>
        Math.pow(2, newNumber))
  );
  return newArray;
};
...
let findMaleAndFemale = (data) => {
  ...
  let newArray = [];
  data.map(newData => {
    if (newData.gender === 'male' || newData.gender === 'female') {
      newArray.push(newData.name);
    }
  });
